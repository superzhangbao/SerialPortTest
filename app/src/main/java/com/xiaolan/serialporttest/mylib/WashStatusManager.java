 ...
 * 洗衣机状态管理类
 */
public class WashStatusManager {
    private static final String TAG = "WashStatusManager";
    ...
    public WashStatusManager() {
        if (mLogmsg == null) {
            mLogmsg = new StringBuilder();
        }
    }

    public WashStatusEvent analyseStatus(String s, byte[] msg) {
        switch (s) {
            case "DeviceAction.JuRen":
                return null;
            case "DeviceAction.JuRenPro":
                if (msg.length == 30) {
                    //洗衣模式
                    mWashMode = (msg[2] & 0xf);
                    //洗涤灯
